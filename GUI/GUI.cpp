...
void GUI::PrintMessage(string msg,int i,int x) const	// function overloading for print message to print multilines //Prints a message on status bar
{


	pWind->SetPen(DARKRED);
	pWind->SetFont(15, BOLD , BY_NAME, "Arial");   
	pWind->DrawString(10+(300*x), WindHeight - (int) (StatusBarHeight-(i*10)), msg); // You may need to change these coordinates later 
	// to be able to write multi-line
	// i is number of lines //x is the number of columnes
}
//////////////////////////////////////////////////////////////////////////////////////////
...
void GUI::DrawMenu() const
{
	...
	MenuItemImages[MENU_LOAD] = "images\\MenuItems\\document.jpg";
	MenuItemImages[MENU_intractive] = "images\\MenuItems\\cursor.jpg";
	MenuItemImages[MENU_auto] = "images\\MenuItems\\play-button.jpg";
	MenuItemImages[MENU_file] = "images\\MenuItems\\documeent.jpg";
...
void GUI::DrawCastle() const
{
	...
	image cas = "images\\MenuItems\\Bondo2a.jpg";
	pWind->DrawImage(cas, CastleStartX, CastleStartY, 240, 240);
	...
	pWind->DrawString(CastleStartX + 210, CastleStartY + 15, "B");
	pWind->DrawString(CastleStartX + 210, CastleEndY - 35, "C");
...
actions GUI::getaction()
{
	Point p;
	GetPointClicked(p);
	if (p.y >= 0 && p.y < MenuBarHeight)
	{
		int ClickedItemOrder = (p.x / MenuItemWidth);
		switch (ClickedItemOrder)
		{
		case MENU_LOAD:
			return addfile;
		case MENU_intractive:
